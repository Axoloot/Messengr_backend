...
describe('Projects', () => {
  ...
  test('Get a Project Pot', async () => {
    const res = await request(app)
      .get(`/pots/projects/${project.id}`)
    expect(res.statusCode).toBe(200);
  });

  test('Create a Project as Partner', async () => {
  ...
  test('Create a Project Pot', async () => {
    const res = await request(app)
      .post(`/pots/projects/${project.id}`)
      .send({
        amount: 1000,
        name: `${uuidv4()}pot`,
